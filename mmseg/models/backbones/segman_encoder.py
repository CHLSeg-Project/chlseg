...
# --- CUDA-only deps: import lazily so the file can be loaded on machines
# without natten / selective_scan / triton (e.g. local Windows dev box). The
# actual GPU training environment provides them. ---
try:
    from natten.functional import na2d, na2d_av, na2d_qk
except Exception:  # pragma: no cover - missing on non-CUDA dev hosts
    na2d = na2d_av = na2d_qk = None

try:
    from .csm_triton import CrossScanTriton, CrossMergeTriton
except Exception:  # pragma: no cover
    CrossScanTriton = CrossMergeTriton = None
