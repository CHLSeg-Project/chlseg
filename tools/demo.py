...
def overlay_mask_on_image(image_rgb: np.ndarray, mask_colored: np.ndarray,
    ...
    overlayed = cv2.addWeighted(image_rgb, 1 - alpha, mask_colored, alpha, 0)
    return overlayed

def main():
    parser = argparse.ArgumentParser(description="Colorize and save segmentation results individually")
    parser.add_argument('--images', default='images', help='Directory of original images')
    parser.add_argument('--masks', default='masks', help='Directory of ground truth masks')
    ...
    parser.add_argument('--overlay', action='store_true', default=True,
                        help='Overlay original image on segmentation results (enabled by default)')
    parser.add_argument('--alpha', type=float, default=0.5,
                        help='Transparency of overlaid mask (0-1), default 0.5')
